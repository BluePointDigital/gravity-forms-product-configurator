 ...
 * @return string
 */
    public function get_field_input( $form, $value = '', $entry = null, $form_id = 0, ...$args ) {
        $form_id  = $form_id ? absint( $form_id ) : ( isset( $form['id'] ) ? absint( $form['id'] ) : 0 );
        $field_id = isset( $args[0] ) && is_numeric( $args[0] ) ? absint( $args[0] ) : absint( $this->id );
        $html_id  = isset( $args[1] ) && is_string( $args[1] ) && '' !== $args[1] ? $args[1] : $this->get_input_id( $form_id );
 ...
 * @return string
 */
    public function get_value_entry_detail( $value, $currency = '', $use_text = false, $format = 'html', $media = 'screen', ...$args ) {
if ( empty( $value ) ) {
